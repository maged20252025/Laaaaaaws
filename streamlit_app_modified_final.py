...
def run_main_app():
    ...
    if not os.path.exists(parent_dir):
        st.error("⚠️ المجلد الرئيسي 'laws/' غير موجود.")
        return

    subfolders = [f.path for f in os.scandir(parent_dir) if f.is_dir()]
    if not subfolders:
        st.warning("📂 لا توجد مجلدات قوانين داخل المجلد الرئيسي.")
        return
...
""", unsafe_allow_html=True)
            st.text_area("📋 المادة كاملة:", value=r["plain"], height=200)

        export_text = "\n\n".join([f'{r["law"]} - المادة {r["num"]}\n{r["plain"]}' for r in filtered])
        b64 = base64.b64encode(export_text.encode()).decode()
        href = f'<a href="data:file/txt;base64,{b64}" download="نتائج_البحث.txt">📥 تحميل النتائج كملف نصي</a>'
